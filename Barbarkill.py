...
# Hard-coded - Only works while you're in pyprog-env environment right now
# IMPORTANT! - Make a setup for this project for the next release
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# You have to be in pyprog-env environment in bash terminal in order to run the program
# To do this just simply write this in terminal --> "conda activate pyprog-env && barbarkill"
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# [NOT DONE] Terminallerin boyutuna veya yazı tipine göre yazıları ayarlama yap
# [HALF DONE] Damage leri ayarla
# [KIND OF DONE] Survival Mode modunu tamamen entegre et
# [KIND OF DONE] Tur sistemi kur
# [DONE] Class ları ayrı modül olarak ayır ve import et projeye
# [DONE] gain_experience() metodunu actions() metodunun içine nested function olarak entegre et
...
# [DONE] Oyun bittiğinde kazandın veya kaybettin ayrımını implent et
# [DONE] Oyunu tekrara sokmak için bir metod oluştur
# [CANCELLED] actions() metoduna ikinci parametre eklendi ona göre metodu tekrar ayarla
#  ---- KNOWN BUGS ----
# (??) [FIXED] [BUG] Düşman ölünce saldırmaya devam ediyor bir dahaki saldırma yaptığında ölüp listeden siliniyor
# [FIXED] [BUG] Oyuncunun yüzdelik sağlık barı, maksimum can 100'ün katsayısı olmadığı zaman sapıtıyor
# [FIXED] [BUG] Düşmanlar her tur iyileşmiyor veya -1 iyileşiyor diye gözüküyor
# -------------------------
